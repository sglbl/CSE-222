...
public class AVLTree < E
  ...
  private void decrementBalance(AVLNode < E > node) {
    ...
    private class SGIterator implements Iterator<E>{
        Node<E> iter = root;
        Stack<Node<E>> stackHolder = new Stack<Node<E>>();

        public SGIterator(){    
            while(iter != null){
                stackHolder.push(iter);
                iter = iter.left;
            }
        }
        ...
        @Override
        public E next() {
            Node<E> temp = stackHolder.pop(); //removing element and storing for adding again
            E element = temp.data;
            if(temp.right != null){
                temp= temp.right;
                while(temp != null){
                    stackHolder.push(temp); //adding same element again.
                    temp = temp.left;
                }
            }
