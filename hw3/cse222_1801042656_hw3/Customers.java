...
public class Customers extends Users implements CustomersInterface {
    ...
    public int whichBranchIsIn(Company c, Products.Product productType, Colors color, int model){
        ...
        for(int i=0; i<c.getAllBranches().size(); i++)
            if( c.getAllBranches().get(i).findProductIndex(productType, color, model) != -1){
                ...
                System.out.println("Product found in " + c.getAllBranches().get(i).getBranchName() + " store / stock is " + stock +  " and product index is " + c.getAllBranches().get(i).findProductIndex(productType, color, model));
    ...
    public void shopOnline(String address, int phoneNumber, Company c, String branchName, int productIndex){
        ...
        for(i=0; i<c.getAllBranches().size(); i++){
            if( c.getAllBranches().get(i).getBranchName().equals(branchName) ){
                c.getAllBranches().get(i).getProducts().get(productIndex).decreaseStock();//decreasing stock 1.
                break;
            }
        }
        previousOrders.add("N|Order-> Branch: " + branchName + " " + c.getAllBranches().get(i).getProducts().get(productIndex).toString() + "Address: "+ address+ " Phone: " + phoneNumber); //N means not supplied.
        System.out.println("N|Order-> Branch: " + branchName + " " + c.getAllBranches().get(i).getProducts().get(productIndex).toString() + "Address: "+ address+ " Phone: " + phoneNumber);
    }
    ...
    public String toString(){
        return "Name: " + name + " Surname: " + surname + " Email: " + email + " Customer Number: " + customerNumber;
    }
