...
public class BranchEmployees extends Users implements BranchEmployeesInterface{
    ...
    public void addNewOrderForCustomer(Customers customer, Products product, String address, int phoneNumber){
        ...
        for(int i=0; i< branchIn.getProducts().size(); i++){
            if( branchIn.getProducts().get(i) == product ){
                productIndex = i;
            }
        }
        if(productIndex == -1){
        ...
        customer.getPreviousOrders().add("Order-> Branch: " + branchIn.getBranchName() + " " + branchIn.getProducts().get(productIndex).toString() + "Address: "+ address+ " Phone: " + phoneNumber);
        branchIn.getProducts().get(productIndex).decreaseStock();
    }
    ...
    public void updatePreviousOrders(Company c, String branchName, int index, int productIndex, Customers cust){
        cust.getPreviousOrders().add("S|Order-> Branch: " + branchName + " " + c.getAllBranches().get(index).getProducts().get(productIndex).toString() ); //N means not supplied.
