...
public class HashTableChainwithLinkedList<K, V> implements KWHashMap<K, V> {
    ...
    // Constructor
    public HashTableChainwithLinkedList() {
   ...
   */
    ...
    public void rehash(){
        LinkedList< Entry<K,V> >[] tableTemp;
        tableTemp = table;
    ...
    public V remove(Object key){
        ...
        if(table[index] == null){
            System.out.println("||||TABLE NULLL");  return null; //Key is not in the table.
        }
        ...
        for(int i=0; i<table[index].size(); i++){
            Entry<K,V> temp = table[index].get(i);
            if(temp.key.equals(key) == true){
                tempDelete = table[index].remove();
                //System.out.println( table[index].remove().value );
                System.out.println("->" + tempDelete.getValue());
